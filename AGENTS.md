...
- **Demo MCP server** (`symfony/mcp-bundle`): `symfony console mcp:server` — exposes tools/prompts/resources/resource-templates from `src/Mcp/`. Not listed in `mcp.json`; start manually when demoing. Test with `{"method":"tools/list","jsonrpc":"2.0","id":1}`.
